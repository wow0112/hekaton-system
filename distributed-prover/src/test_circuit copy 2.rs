...
impl<F: PrimeField> ZkDbSqlCircuit<F> {
    ...
    pub fn rand<R: Rng>(mut rng: R, params: &ZkDbSqlCircuitParams) -> Self {
        ...
        let right_shipdate =F::from(15u64);
...
impl<F: PrimeField> CircuitWithPortals<F> for ZkDbSqlCircuit<F> {
    ...
    fn get_serialized_witnesses(&self, subcircuit_idx: usize) -> Vec<u8> {
        ...
        if subcircuit_idx < self.params.num_rows {
            // for &field in &self.lineitem_table[subcircuit_idx] {
    ...
    fn set_serialized_witnesses(&mut self, subcircuit_idx: usize, bytes: &[u8]) {
        ...
        if subcircuit_idx < self.params.num_rows {
            ...
            // for j in 0..7 {
            //     self.lineitem_table[subcircuit_idx][j] = F::deserialize_uncompressed_unchecked(&bytes[j * field_size..(j + 1) * field_size]).unwrap();
            // }
            // self.right_shipdate = F::deserialize_uncompressed_unchecked(&bytes[7 * field_size..8 * field_size]).unwrap();
            self.right_shipdate = F::deserialize_uncompressed_unchecked(bytes).unwrap();
        }else{
            return;
        }
        println!("@@@@@@@@@@@linetable:{:?}",self.lineitem_table);
        println!("@@@@@@@@@@@right:{:?}",self.right_shipdate);
    }
    ...
    fn generate_constraints<P: PortalManager<F>>(
        ...
        if subcircuit_idx < self.params.num_rows  {
            ...
            // // println!("rhs_var:{:?}",rhs_var.value()?);
            // // println!("lhs_var:{:?}",lhs_var.value()?);
            // // if is_less_than_var.value()? == true {
            // //     println!("^^^^^circuit {subcircuit_idx} is less than");
            // // }else{
            // //     println!("^^^^^circuit {subcircuit_idx} is not less than");
            // // }
            ...
            pm.set(format!("{subcircuit_idx}"), &rhs_var)?;

            let elapsed = start.elapsed();
            println!("circuit {subcircuit_idx} Elapsed time: {:?}", elapsed);
        }else{
            for i in 0..self.params.num_rows {
                let _ = pm.get(&format!("{i}"))?;
            }
        }
    ...
    fn get_portal_subtraces(&self) -> Vec<Vec<crate::transcript::TranscriptEntry<F>>> {
        ...
        for (i, lhs_row) in self.lineitem_table.iter().enumerate() {
            ...
            let rhs_var: FpVar<F> = FpVar::new_constant(ns!(cs, "rhs_var"),  &self.right_shipdate).unwrap();
            let _ = pm.set(format!("{i}"), &rhs_var);
        }
        pm.start_subtrace(ConstraintSystem::new_ref());
        for i in 0..self.params.num_rows {
            let _ = pm.get(&format!("{i}"));
